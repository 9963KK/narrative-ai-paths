...
-- 添加用户等级字段
ALTER TABLE users ADD COLUMN IF NOT EXISTS user_level VARCHAR(10) DEFAULT 'basic';
...
-- 更新现有的 standard 等级为 advanced
UPDATE system_model_pool SET performance_level = 'advanced' WHERE performance_level = 'standard';
...
CREATE TABLE IF NOT EXISTS user_level_permissions (
    ...
    description TEXT,
    max_daily_requests INTEGER DEFAULT NULL, -- 每日最大请求数限制
    max_tokens_per_request INTEGER DEFAULT NULL, -- 单次请求最大token数
...
CREATE TABLE IF NOT EXISTS user_level_changes (
    ...
    changed_by UUID REFERENCES users(id), -- 执行变更的管理员
    reason TEXT, -- 变更原因
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);
...
-- ==========================================
-- 5. 更新存储过程 - 基于用户等级获取可用模型
-- ==========================================

-- 删除旧的函数
DROP FUNCTION IF EXISTS get_user_available_models(uuid);
...
-- 为用户等级权限表添加更新时间戳触发器
DROP TRIGGER IF EXISTS update_user_level_permissions_updated_at ON user_level_permissions;
CREATE TRIGGER update_user_level_permissions_updated_at
    BEFORE UPDATE ON user_level_permissions
    FOR EACH ROW
